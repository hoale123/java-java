...
public class DatingController {
    ...
    //Teapot
    @RequestMapping(value = "/users/{id}/are_they_a_teapot", method =RequestMethod.GET)
    @ResponseStatus(HttpStatus.I_AM_A_TEAPOT)
    public String isUserATeapot(@PathVariable int id){
        if (id < 7){
            return "Yes, Adam is a teapot";
        }else {
            return "The response code for i am a teapot";
        }
    }
    //INDEX retrieve person by id
