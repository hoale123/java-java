...
public class RecordStoreControllerTest {
    ...
    public void shouldReturn422StatusCodeForInvalidRequestBody()throws Exception{
        Record inputRecord = new Record();
        inputRecord.setArtist("MGK");
        //convert to json
        String json = mapper.writeValueAsString(inputRecord);
        //.content(json).contentType(MediaType.APPLICATION_JSON) 415
        mockMvc.perform(post("/records").content(json).contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isUnprocessableEntity());
    }

    @Test
    public void shouldReturn422StatusCodeForMismatchId()throws Exception{
        Record inputRecord = new Record();
        inputRecord.setArtist("The Beach Boys");
        inputRecord.setAlbum("Pet Sounds");
        inputRecord.setYear("2012");
        inputRecord.setId(2);
        //convert to json
        String json = mapper.writeValueAsString(inputRecord);
        //.content(json).contentType(MediaType.APPLICATION_JSON) 415
        mockMvc.perform(put("/records/1").content(json).contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isUnprocessableEntity());
    }
