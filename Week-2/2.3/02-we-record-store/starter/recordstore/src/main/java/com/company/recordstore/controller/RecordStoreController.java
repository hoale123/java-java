...
public class RecordStoreController {
    ...
    private static List<Record> recordList = new ArrayList<>(Arrays.asList(
            new Record("The Beach Boys", "Pet Sounds","2012", idCounter++),
            new Record("Billy Joel", "The Stranger","2013", idCounter++),
            new Record("The Beatles", "Revolver","2014", idCounter++),
            new Record("Kanye West", "My Beautiful Dark Twisted Fantasy","2015", idCounter++),
            new Record("Sturgill Simpson", "Metamodern Sounds in Country Music","2016", idCounter++)
    ));
    ...
    public void updateRecordById(@PathVariable int id, @RequestBody Record record) {
        ...
        if (record.getId() != id){
            throw new IllegalArgumentException("Id in parameter must match the id of the body");
        }
        int index = -1;
